...
from core.year_resolver import resolve_year_from_path


from config.paths import RAW_DIR


RAW = RAW_DIR / "raw_art94_2024.json"
...
def normalize_ganancias_escalas():
    ...
    if isinstance(data, list):
        tramos = data
        anio = resolve_year_from_path(RAW)

        url = None
        fuente = "ARCA"
    else:
        tramos = data.get("tramos", [])
        anio = resolve_year_from_path(RAW) or data.get("anio") or data.get("year")
